...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
	...
	public ResponseEntity<Problem> handleAccessDenied(AccessDeniedException ex, NativeWebRequest request) {
		Problem problem = Problem.builder().withStatus(Status.FORBIDDEN).with(MESSAGE_KEY, "errorUserAccessDenied").build();
		return create(ex, problem, request);
	}
	...
	public ResponseEntity<Problem> handleBadRequestAlertException(BadRequestAlertException ex,
		...
		HttpHeaders httpHeaders = createFailureAlert(applicationName, true, ex.getEntityName(), ex.getErrorKey(), ex.getMessage());
		return create(ex,request,httpHeaders);
	}
	
	@ExceptionHandler
	public ResponseEntity<Problem> handleBadCredentialException(BadCredentialsException ex, NativeWebRequest request) {
		HttpHeaders httpHeaders = createFailureAlert(applicationName, true, ex.getEntityName(), ex.getErrorKey(), ex.getMessage());
		return create(ex,request,httpHeaders);
	}
